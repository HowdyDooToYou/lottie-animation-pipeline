...
MotionProof treats provider and imported Lottie JSON as untrusted input. The
...
or the operator explicitly sets `MOTIONPROOF_CHROMIUM_NO_SANDBOX=1`.

Do not disable those controls to make a candidate pass. A candidate that
depends on executable or external content is outside the MotionProof contract.
