...
Soliddd supports four integration levels. Choose the smallest surface the host
can use.
...
```bash
npx soliddd-motion create "A calm save confirmation" --json
```
...
```json
{
  "mcpServers": {
    "soliddd-motion": {
      "command": "npx",
      "args": ["--yes", "soliddd-motion@2.0.0", "mcp"]
    }
  }
}
```
...
```text
/soliddd-motion:create-motion Create a restrained onboarding success state.
```
...
cp -R ./plugins/soliddd-motion/skills/create-motion .agents/skills/
```
...
cp -R ./plugins/soliddd-motion/skills/create-motion .claude/skills/
```
...
Agents integrating Soliddd must:
...
7. keep provider credentials outside Soliddd requests and artifacts; and
