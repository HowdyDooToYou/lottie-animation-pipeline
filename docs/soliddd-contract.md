...
Soliddd is a fail-closed compiler boundary for production motion. Providers
...
If an existing target is present, Soliddd fails unless the caller explicitly
...
`animation.json`
: Strict Lottie source with `meta.soliddd` and accessibility provenance.
