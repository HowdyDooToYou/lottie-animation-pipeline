...
} satisfies Record<keyof SolidddTheme, number[][]>;
...
export function buildRecipeCandidate(request: SolidddRequest): MotionCandidate {
...
function applyTheme(animation: Record<string, unknown>, theme: SolidddTheme): void {
  visit(animation, (value) => {
    if (!Array.isArray(value) || value.length < 3) return;
    if (!value.slice(0, 3).every((channel) => typeof channel === 'number')) return;

    for (const role of Object.keys(theme) as Array<keyof SolidddTheme>) {
