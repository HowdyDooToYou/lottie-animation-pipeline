...
export interface SolidddRenderResult {
  probe: RenderProbeResult;
  poster: Buffer;
  playerSource: string;
}

export interface SolidddRenderOptions {
  chromiumPath?: string;
  posterFrame: number;
}
...
export async function renderSolidddCandidate(
  animation: Record<string, unknown>,
  options: SolidddRenderOptions,
): Promise<SolidddRenderResult> {
...
function chromiumLaunchArguments(): string[] {
  ...
  if (runningAsRoot || process.env.SOLIDDD_CHROMIUM_NO_SANDBOX === '1') {
