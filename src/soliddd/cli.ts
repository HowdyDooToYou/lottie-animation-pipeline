...
import type { SolidddRequestInput, SolidddResult, SolidddTheme } from './contracts.ts';
...
const HELP = `Soliddd — motion that ships

Usage:
  soliddd create "A calm checkout success state" [options]
  soliddd certify ./candidate.json --prompt "Describe the intended motion" [options]
  soliddd recipes [--json]
  soliddd mcp
...
Options:
  ...
  --out <directory>           Output root (default: ./soliddd-output)
...
export async function runCli(
  ...
  try {
    ...
    if (json) {
      ...
      io.stdout.write(`\nSOLIDDD · ${result.certification.score}/100 · certified\n`);
      io.stdout.write(`${result.outputDirectory}\n\n`);
      for (const artifact of result.artifacts) {
        io.stdout.write(`  ${artifact.kind.padEnd(10)} ${artifact.relativePath}\n`);
      }
    } else {
      io.stderr.write(`\nSoliddd stopped at ${result.stage}; no production bundle was promoted.\n`);
...
function buildTheme(parsed: ParsedFlags): SolidddTheme {
  const theme: SolidddTheme = {};
