...
function assertPortableCandidate(animation: Record<string, unknown>): void {
  ...
  if (violations.length > 0) {
    throw new Error(
      'Soliddd accepts portable, expression-free vector Lottie only. '
...
export function certifyCandidate(
  ...
  posterBytes: number,
): { certification?: SolidddCertification; issues: SolidddIssue[] } {
  const checks: SolidddCheck[] = [
    {
      id: 'schema',
  ...
  const issues: SolidddIssue[] = checks
...
export function qualityIssues(report: QualityReport): SolidddIssue[] {
  return [
